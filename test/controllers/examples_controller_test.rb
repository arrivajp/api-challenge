...
class ExamplesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create example" do
    assert_difference('Example.count') do
      post examples_url, params: { example: { english: @example.english, japanese: @example.japanese } }
    end
  ...
  test "should update example" do
    patch example_url(@example), params: { example: { english: @example.english, japanese: @example.japanese } }
